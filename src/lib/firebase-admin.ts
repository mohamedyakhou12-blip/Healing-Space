...
function initializeAdmin() {
  ...
  if (sa && sa.private_key && sa.private_key.includes("-----BEGIN")) {
    try {
      ...
      adminApp = initializeApp(
        { 
          projectId: sa.project_id || "healing-space-5a76f",
          storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || `${sa.project_id || "healing-space-5a76f"}.firebasestorage.app`,
        },
        ADMIN_APP_NAME
      );
