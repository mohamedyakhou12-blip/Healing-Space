    ...
    if (adminId) {
      ...
      const cleanupPromise = (async () => {
        try {
          ...
          // silently ignores `updatedAt`. Fetching with `updatedAt: { lt }`
          // therefore deleted ALL rejected payments on every admin GET.
          // Fix: query rejected only, then filter by age in JS.
          const rejected = await db.payment.findMany({
