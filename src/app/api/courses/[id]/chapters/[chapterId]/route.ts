...
export async function PUT(
  ...
  // Rate limiting
  const rlKey = rateLimitKey(request, "courses-chapters-put");
  ...
  try {
    ...
    const chapter = await db.courseChapter.update({ where: { id: chapterId }, data: parsed.data });
...
export async function DELETE(
  ...
  // Rate limiting
  const rlKey = rateLimitKey(request, "courses-chapters-delete");
  if (isRateLimited(rlKey, { max: 15, windowMs: 60_000 })) {
    return NextResponse.json(
      { error: "Too many requests. Please try again later.", success: false },
      { status: 429 }
